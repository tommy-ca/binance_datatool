...
class LocalStorage(BaseStorage):
    ...
    def __init__(self, settings: Settings):
        ...
        self.base_path = settings.local_data_dir
    ...
    async def optimize_partitions(
        ...
        try:
            partitions = await self.list_partitions(
                zone, exchange, data_type, trade_type, symbol
            )
            ...
            return {
                ...
                "recommendations": [
                    f"Consider merging {len(small_files)} small files" if small_files else "No optimization needed"
                ]
            }
    ...
    def _filter_partitions(
        ...
        if start_date:
            filtered = [p for p in filtered if p.get('date', datetime.min.date()) >= start_date.date()]
        
        if end_date:
            filtered = [p for p in filtered if p.get('date', datetime.max.date()) <= end_date.date()]
