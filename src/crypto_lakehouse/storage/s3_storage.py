...
class S3Storage(BaseStorage):
    ...
    def _init_s3_clients(self):
        ...
        session_kwargs = {"region_name": self.s3_config.region}
    ...
    async def list_partitions(
        ...
        try:
            ...
            response = self.s3_client.list_objects_v2(
                Bucket=self.s3_config.bucket_name, Prefix=prefix, Delimiter="/"
            )
            ...
            for prefix_info in response.get("CommonPrefixes", []):
                partition_path = prefix_info["Prefix"].replace(
                    f"{self.base_path}/{table_path}/", ""
                )
        ...
        try:
            ...
            if "Contents" in response:
                ...
                self.s3_client.delete_objects(
                    Bucket=self.s3_config.bucket_name, Delete={"Objects": delete_keys}
                )
...
class GlueDataCatalog(DataLakeMetadata):
    ...
    def __init__(self, settings: Settings):
        self.settings = settings
        self.glue_client = boto3.client("glue", region_name=settings.data_catalog.region)
        ...
        try:
            ...
            for col in partition_columns:
                if col in schema:
                    ...
                    partition_keys.append({"Name": col, "Type": glue_type})
