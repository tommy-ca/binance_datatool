...
from crypto_lakehouse.core.models import MarketType, DataType, WorkflowResult
...
from crypto_lakehouse.utils.query_engine import QueryEngine
...
async def complete_pipeline_workflow(
    ...
    **kwargs
    ...
    try:
            ...
            for market_type in market_types:
                for attempt in range(max_retries + 1):
                    try:
                        ...
                        if attempt < max_retries and error_recovery:
                            logger.warning(f"Generate klines attempt {attempt + 1} failed, retrying: {e}")
        ...
        overall_success = all(
            isinstance(r, list) and all(item.success for item in r) if isinstance(r, list) else r.success
