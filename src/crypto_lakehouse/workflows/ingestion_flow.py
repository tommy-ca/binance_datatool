...
from ..core.models import DataIngestionTask
...
async def ingest_data_task(task: DataIngestionTask, settings: Settings) -> Dict[str, Any]:
    ...
    try:
        ...
        if task.data_type.value == "klines":
            async for data in ingestor.ingest_klines(
                task.symbols, task.interval, task.trade_type, task.start_date, task.end_date
            ):
            ...
            async for data in ingestor.ingest_funding_rates(
                task.symbols, task.trade_type, task.start_date, task.end_date
            ):
...
async def validate_data_quality(data_info: Dict[str, Any], settings: Settings) -> Dict[str, Any]:
...
async def update_data_catalog(
    task: DataIngestionTask, data_info: Dict[str, Any], settings: Settings
) -> bool:
...
async def ingestion_flow(task: DataIngestionTask, settings: Settings) -> Dict[str, Any]:
    ...
    try:
        ...
        await update_metadata_record(metadata, "failed", errors=[str(e)])
...
class IngestionFlow(BaseWorkflow):
    ...
    def get_flow_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "version": "2.0.0",
            "task_runner": ConcurrentTaskRunner(
                concurrency=self.settings.workflow.concurrency_limit
            ),
