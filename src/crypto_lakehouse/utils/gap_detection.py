...
class GapDetector:
    ...
    def detect_gaps(
        ...
        try:
            ...
            logger.info(
                f"Detected {len(gaps)} gaps across {len(sorted_data[symbol_column].unique())} symbols"
            )
            return gaps
    ...
    def _detect_gaps_for_symbol(
        self, data: pl.DataFrame, symbol: str, interval_delta: timedelta, time_column: str
    ) -> List[Dict[str, Any]]:
    ...
    def generate_missing_timestamps(
        self, gaps: List[Dict[str, Any]], interval: Interval
    ) -> List[Dict[str, Any]]:
        """
        ...
        for gap in gaps:
            ...
            while current_time < end_time:
                missing_timestamps.append(
                    {"symbol": symbol, "timestamp": current_time, "gap_info": gap}
                )
    ...
    def create_gap_report(self, gaps: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for gap in gaps:
            ...
            symbol_summary[symbol]["largest_gap"] = max(
                symbol_summary[symbol]["largest_gap"], gap["missing_periods"]
            )
    ...
    def validate_data_completeness(
        ...
        try:
            ...
            for symbol in data[symbol_column].unique():
                ...
                symbol_gaps = self.detect_gaps(symbol_data, interval, time_column, symbol_column)
                ...
                symbols_complete[symbol] = {
                    ...
                    "covers_expected_range": (
                        symbol_start <= expected_start and symbol_end >= expected_end
                    ),
                }
