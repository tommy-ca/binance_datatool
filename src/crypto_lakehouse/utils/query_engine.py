        ...
        try:
            ...
            if self.config.enable_s3 and self.settings.s3.enabled:
                ...
                if self.settings.s3.access_key_id and self.settings.s3.secret_access_key:
                    self.connection.execute(f"SET s3_access_key_id='{self.settings.s3.access_key_id}'")
                    self.connection.execute(f"SET s3_secret_access_key='{self.settings.s3.secret_access_key}'")
...
def create_query_engine(settings: Settings, config: Optional[QueryConfig] = None) -> DuckDBQueryEngine:
    """
