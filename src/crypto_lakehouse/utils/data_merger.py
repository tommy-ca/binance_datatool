...
class MergeConfig(BaseModel):
    ...
    merge_strategy: MergeStrategy = Field(
        default=MergeStrategy.BULK_PRIORITY, description="Strategy for handling overlapping data"
    )
    conflict_resolution: ConflictResolution = Field(
        default=ConflictResolution.OVERWRITE, description="How to resolve data conflicts"
    )
    tolerance_ms: int = Field(
        default=1000, description="Tolerance in milliseconds for timestamp matching"
    )
...
class DataMerger:
    ...
    def merge_kline_data(
        self, bulk_data: List[KlineData], incremental_data: List[KlineData], symbol: str
    ) -> MergeResult:
        """
    ...
    def merge_funding_rate_data(
        self, bulk_data: List[FundingRateData], incremental_data: List[FundingRateData], symbol: str
    ) -> MergeResult:
        """
    ...
    def _funding_rates_to_df(
        self, funding_rates: List[FundingRateData], source: str
    ) -> pl.DataFrame:
    ...
    def _detect_overlaps(self, bulk_df: pl.DataFrame, incremental_df: pl.DataFrame) -> DataOverlap:
        ...
        if overlap_start <= overlap_end:
            ...
            bulk_overlap = bulk_df.filter(
                (pl.col("open_time") >= overlap_start) & (pl.col("open_time") <= overlap_end)
            )
    ...
    def _merge_dataframes(
        self, bulk_df: pl.DataFrame, incremental_df: pl.DataFrame, overlap_info: DataOverlap
    ) -> pl.DataFrame:
    ...
    def _merge_bulk_priority(
        self, bulk_df: pl.DataFrame, incremental_df: pl.DataFrame
    ) -> pl.DataFrame:
        ...
        if not incremental_df.is_empty():
            ...
            incremental_new = incremental_df.filter(~pl.col(time_col).is_in(bulk_times))
    ...
    def _merge_incremental_priority(
        self, bulk_df: pl.DataFrame, incremental_df: pl.DataFrame
    ) -> pl.DataFrame:
        ...
        if not bulk_df.is_empty():
            ...
            bulk_new = bulk_df.filter(~pl.col(time_col).is_in(inc_times))
