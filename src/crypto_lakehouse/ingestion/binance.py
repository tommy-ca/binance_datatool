...
from .bulk_downloader import S5cmdDownloader, BinanceDataParser
    ...
    async def ingest_klines(
        ...
        **kwargs
        ...
        if not end_date or end_date > bulk_cutoff:
            ...
            logger.info(f"Using incremental ingestion for {incremental_start} to {end_date or 'now'}")
        ...
        try:
            ...
            while current_date <= end:
                ...
                if data_type == DataType.KLINES:
                    ...
                    file_url = f"{self.base_url}/{base_path}/{symbol}-liquidationSnapshot-{date_str}.zip"
    ...
    async def _get_exchange(self) -> ccxt.binance:
        ...
        if self._exchange is None:
            config = {
                ...
                'options': {
                    'defaultType': 'spot'  # Will be overridden as needed
                }
            }
