...
from crypto_lakehouse.core.models import DataType, MarketType
...
class TestAPIDownloadWorkflow:
    ...
    async def test_api_download_missing_klines(self, config, mock_binance_client):
        ...
        # Mock API client
        mock_binance_client.return_value.fetch_ohlcv = AsyncMock(
            return_value=[
    ...
    async def test_api_download_recent_funding_rates(self, config, mock_binance_client):
        ...
        # Mock API client
        mock_binance_client.return_value.fetch_funding_rate_history = AsyncMock(
            return_value=[
...
class TestCompletePipelineWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_pipeline_matching_legacy_sequence(
        self, config, mock_s3_client, mock_binance_client
    ):
    ...
    async def test_complete_pipeline_error_recovery(self, config):
        ...
        with patch(
            "crypto_lakehouse.workflows.prefect_workflows.aws_download_workflow"
        ) as mock_download:
