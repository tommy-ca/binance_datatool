...
class TestKlineProcessor:
    ...
    async def test_process_klines(self, test_settings, sample_kline_dataframe):
        ...
        result = await processor.process(
            data=sample_kline_dataframe, source_zone=DataZone.BRONZE, target_zone=DataZone.SILVER
        )
    ...
    async def test_calculate_additional_indicators(self, test_settings, sample_kline_dataframe):
        ...
        enhanced = await processor.calculate_additional_indicators(
            larger_data, indicators=["sma_20", "ema_20", "bollinger_bands"]
        )
...
class TestFundingRateProcessor:
    ...
    async def test_process_funding_rates(self, test_settings):
        ...
        result = await processor.process(
            data=funding_data, source_zone=DataZone.BRONZE, target_zone=DataZone.SILVER
        )
        ...
        funding_data = pl.DataFrame(
            {
                "symbol": ["BTCUSDT"] * 100,
                "funding_time": [
                    datetime(2024, 1, 1, 0, 0, tzinfo=timezone.utc) + pd.Timedelta(hours=8 * i)
                    for i in range(100)
                ],
        ...
        funding_data = pl.DataFrame(
            {
                ...
                "funding_rate": [0.0001 if i % 2 == 0 else -0.0001 for i in range(100)],
